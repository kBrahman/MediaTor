...
public class HexHiveView<T> extends View {
    ...
    public HexHiveView(Context context, @Nullable AttributeSet attrs) {
        ...
        int fullColor = typedArray.getColor(R.styleable.HexHiveView_hexhive_fullColor, r.getColor(R.color.basic_blue_highlight_dark));
        typedArray.recycle();
    ...
    private void initPaints(int borderColor, int emptyColor, int fullColor) {
    ...
    public void updateData(HexDataAdapter<T> hexDataAdapter) {
        ...
        if (DP == null && canvasHeight > 0 && canvasWidth > 0 && hexDataAdapter != null) {
            DP = new DrawingProperties(hexDataAdapter, hexagonBorderPaint.getStrokeWidth(), getPaddingLeft(), getPaddingTop(), canvasWidth - getPaddingRight(), canvasHeight - getPaddingBottom());
        }
        if (DP == null) {
        ...
        if (hexDataAdapter != null && hexDataAdapter.getFullHexagonsCount() >= 0 && canvasWidth > 0 && canvasHeight > 0) {
            async(this, HexHiveView::asyncDraw, canvasWidth, canvasHeight, hexDataAdapter, (v, w, h, a) -> v.invalidate());
        }
    }
    ...
    private static final class DrawingProperties {
        ...
        DrawingProperties(HexDataAdapter adapter, float hexBorderWidth, int left, int top, int right, int bottom) {
            ...
            evenRowOrigin = new Point((int) (origin.x + (hexWidth / 2)), (int) (origin.y + (hexHeight / 2)));
            // calculate number of hexagons in an even row
            oddRowOrigin = new Point((int) (evenRowOrigin.x + (hexWidth / 2)), (int) (evenRowOrigin.y + hexHeight));
        }
    }
    ...
    private static void drawHexagon(final DrawingProperties DP, final Canvas canvas, final Paint borderPaint, final CubePaint fillPaint, final boolean drawCube) {
