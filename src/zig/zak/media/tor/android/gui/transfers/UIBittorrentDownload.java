...
import zig.zak.media.tor.android.core.providers.TableFetcher;
import zig.zak.media.tor.android.core.providers.TableFetchers;
...
import zig.zak.media.tor.bittorrent.BTDownload;
import zig.zak.media.tor.bittorrent.BTDownloadItem;
import zig.zak.media.tor.bittorrent.BTDownloadListener;
...
public final class UIBittorrentDownload implements BittorrentDownload {
    ...
    private long calculateSize(BTDownload dl) {
        ...
        if (partial) {
            List<zig.zak.media.tor.transfers.TransferItem> items = dl.getItems();

            long totalSize = 0;
            for (zig.zak.media.tor.transfers.TransferItem item : items) {
                if (!item.isSkipped()) {
