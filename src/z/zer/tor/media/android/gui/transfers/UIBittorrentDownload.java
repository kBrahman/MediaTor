...
import z.zer.tor.media.bittorrent.BTDownloadListener;
...
public final class UIBittorrentDownload implements BittorrentDownload {
    ...
    private long calculateSize(BTDownload dl) {
        ...
        if (partial) {
            List<z.zer.tor.media.transfers.TransferItem> items = dl.getItems();

            long totalSize = 0;
            for (z.zer.tor.media.transfers.TransferItem item : items) {
                if (!item.isSkipped()) {
