...
import z.zer.tor.media.android.gui.transfers.TorrentFetcherDownload;
...
public final class HandpickedTorrentDownloadDialog extends AbstractConfirmListDialog<HandpickedTorrentDownloadDialog.TorrentFileEntry> {
    ...
    public static HandpickedTorrentDownloadDialog newInstance(Context ctx, TorrentInfo tinfo, String magnetUri, long torrentFetcherDownloadTokenId) {
        //
        ...
        dlg.prepareArguments(R.drawable.download_icon, tinfo.name(), ctx.getString(R.string.pick_the_files_you_want_to_download_from_this_torrent), JsonUtils.toJson(torrentInfoList), SelectionMode.MULTIPLE_SELECTION);
    ...
    private static TorrentFileEntryList getTorrentInfoList(FileStorage fileStorage) {
        ...
        if (fileStorage != null && fileStorage.numFiles() > 0) {
            ...
            for (int i = 0; i < n; i++) {
                entryList.add(new TorrentFileEntry(i, fileStorage.fileName(i), fileStorage.filePath(i), fileStorage.fileSize(i)));
            }
        }
        return entryList;
    }
    ...
    protected void initComponents(Dialog dlg, Bundle savedInstanceState) {
        ...
        if (this.torrentInfo == null && arguments != null && (torrentInfoData = arguments.getByteArray(BUNDLE_KEY_TORRENT_INFO_DATA)) != null) {
    ...
    private class HandpickedTorrentFileEntriesDialogAdapter extends ConfirmListDialogDefaultAdapter<TorrentFileEntry> {
        ...
        HandpickedTorrentFileEntriesDialogAdapter(Context context, List<TorrentFileEntry> list, SelectionMode selectionMode) {
    ...
    private static class OnStartDownloadsClickListener implements View.OnClickListener {
        ...
        public void onClick(View v) {
            if (Ref.alive(ctxRef) && Ref.alive(dlgRef)) {
                ...
                List<TorrentFileEntry> checked = (selectionMode == AbstractConfirmListDialog.SelectionMode.NO_SELECTION) ? (List<TorrentFileEntry>) dlg.getList() : new ArrayList<>();
        ...
        private void startTorrentPartialDownload(List<TorrentFileEntry> results) {
            if (!Ref.alive(ctxRef) || !Ref.alive(dlgRef) || results == null || dlgRef.get().getList() == null || results.size() > dlgRef.get().getList().size()) {
                return;
            }
            ...
            Engine.instance().getThreadPool().execute(() -> {
                try {
                    ...
                    BTEngine.getInstance().download(dlg.getTorrentInfo(), null, selection, peers, TransferManager.instance().isDeleteStartedTorrentEnabled());
